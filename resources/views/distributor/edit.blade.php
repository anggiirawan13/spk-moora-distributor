...
@section('title', 'Edit Distributor Barang Elektrikal')
...
@php
    $backUrl = request('return_to') === 'import-approval'
        ...
        : (request('return_to') === 'import-history'
            ...
            : ($distributor->import_batch_id ? route('import.excel.history') : route('distributor.index')));
    $formAction = route('distributor.update', $distributor->id);
            ...
            <div class="card shadow border-0">
                <div class="card-header bg-gradient-primary text-white py-3">
                    <h5 class="mb-0 font-weight-bold">
                        <i class="fas fa-edit mr-2"></i>Edit Data Distributor
                    </h5>
                </div>
                <div class="card-body p-4">
                    <x-alert />

                    <form action="{{ $formAction }}" method="POST" enctype="multipart/form-data" id="distributorForm">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="mb-3">
                                        <p class="mb-1 text-muted">
                                            <i class="fas fa-eye mr-1"></i>Logo Saat Ini:
                                        </p>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('payment_term_id') is-invalid @enderror" 
                                        ...
                                        @foreach ($paymentTerms as $term)
                                            <option value="{{ $term->id }}" {{ old('payment_term_id', $distributor->payment_term_id) == $term->id ? 'selected' : '' }}>
                                                {{ $term->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('delivery_method_id') is-invalid @enderror" 
                                        ...
                                        @foreach ($deliveryMethods as $method)
                                            <option value="{{ $method->id }}" {{ old('delivery_method_id', $distributor->delivery_method_id) == $method->id ? 'selected' : '' }}>
                                                {{ $method->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('business_scale_id') is-invalid @enderror" 
                                        ...
                                        @foreach ($businessScales as $scale)
                                            <option value="{{ $scale->id }}" {{ old('business_scale_id', $distributor->business_scale_id) == $scale->id ? 'selected' : '' }}>
                                                {{ $scale->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-8">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror" 
                                              ...
                                              id="description"
                                              rows="4" 
                                              placeholder="Masukkan deskripsi perusahaan">{{ old('description', $distributor->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    <div class="d-flex justify-content-between align-items-center mt-1">
                                        ...
                                        <small class="text-muted" id="charCount">{{ strlen(old('description', $distributor->description)) }}/1000 karakter</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('is_active') is-invalid @enderror" 
                                        ...
                                        <option value="1" {{ old('is_active', $distributor->is_active) == '1' ? 'selected' : '' }}>Aktif</option>
                                        <option value="0" {{ old('is_active', $distributor->is_active) == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                                    </select>
                                    @error('is_active')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
